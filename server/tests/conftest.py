...
# Automatically clear city collection before each test
@pytest.fixture(autouse=True)
def clear_city_data():
    """
    Clear the city collection before each test.
    This prevents cross-test pollution such as duplicate cities,
    missing cities, and inconsistent PUT/DELETE behavior.
    """
    dc.db.cities.delete_many({})  # Clear before test
    yield
    # Optionally clear after as well for extra safety
    dc.db.cities.delete_many({})